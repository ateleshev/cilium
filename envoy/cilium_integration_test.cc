...
TEST_P(CiliumGoBlocktesterIntegrationTest, CiliumGoBlockParserUpstreamWritesFirst) {
  ...
  fake_upstream_connection->write("24:DROP reply direction\n");
  fake_upstream_connection->write("24:PASS reply direction\n");
...
TEST_P(CiliumGoBlocktesterIntegrationTest, CiliumGoBlockParserPartialBlocks) {
  ...
  fake_upstream_connection->write("24:DROP reply ");
  fake_upstream_connection->write("direction\n24:PASS");
  fake_upstream_connection->write(" reply direction\n");
  tcp_client->waitForData("24:PASS reply direction\n");

  tcp_client->write("27:PASS original direction\n");
  fake_upstream_connection->waitForData(FakeRawConnection::waitForInexactMatch("27:PASS original direction\n"));
...
TEST_P(CiliumGoBlocktesterIntegrationTest, CiliumGoBlockParserInjectPartial) {
  ...
  fake_upstream_connection->write("24:PASS reply");
  tcp_client->write("26:INJECT reply direction\n");
  tcp_client->write("27:PASS original direction\n");
  ...
  // These can in principle arrive in either order
  tcp_client->waitForData("24:PASS reply direction\n", true); // XXX: Envoy inverts 2nd parameter on initial pass
  tcp_client->waitForData("26:INJECT reply direction\n", true); // XXX: Envoy inverts 2nd parameter on initial pass
...
TEST_P(CiliumGoBlocktesterIntegrationTest, CiliumGoBlockParserInjectPartialMultiple) {
  ...
  fake_upstream_connection->waitForData(FakeRawConnection::waitForInexactMatch("29:INSERT original direction\n"));
  fake_upstream_connection->waitForData(noMatch("DROP"));

  fake_upstream_connection->write("24:DROP reply direction\n");
  fake_upstream_connection->write("25:PASS2 reply direction\n");
